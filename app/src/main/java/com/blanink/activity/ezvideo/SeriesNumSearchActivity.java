...
public class SeriesNumSearchActivity extends RootActivity implements OnClickListener/*, OnAuthListener*/ {
    ...
    private void initTitleBar() {
        TextView mTitleBar = (TextView) findViewById(R.id.last);
        mTitle = (TextView) findViewById(R.id.btn_user);
        ;
    ...
    private void addQueryCameraAddVerifyCode() {
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    boolean result = MyApplication.getOpenSDK().addDevice(mSerialNoStr, mVerifyCode);

                    /***********如有需要开发者需要自己保存此验证码***********/
//                    if (!TextUtils.isEmpty(mVerifyCode)) {
//                        //保存密码
//                        MyApplication.getOpenSDK().setValidateCode(mVerifyCode, mSerialNoStr);
//                    }
