...
//    private UnbindDeviceTriggerHelper mTriggerHelper;
    ...
    private int probeDeviceInfo(String deviceSerial) {
        try {
            mEZProbeDeviceInfo = MyApplication.getOpenSDK().probeDeviceInfo(serialNo);
...
//                            LocalInfo.getInstance().setWifiPassword(mac, wifiPassword);
        ...
        MyApplication.getOpenSDK().startConfigWifi(AutoWifiConnectingActivity.this, serialNo, wifiSSID, wifiPassword, mEZStartConfigWifiCallback);
    }
        ...
        new Thread() {
            public void run() {
                ...
                while (count > 0) {
                    ...
                    try {
                        MyApplication.getOpenSDK().addDevice(serialNo, mVerifyCode);
...
//                        if (!TextUtils.isEmpty(mVerifyCode)) {
//                            //保存密码
//                            MyApplication.getOpenSDK().setValidateCode(mVerifyCode, serialNo);
//                        }
